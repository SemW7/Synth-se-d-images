class Trajectoire {
	...
	static rectiligne(scene, depart, arrivee, nbPts, trace, couleur) {
		...
		// On fait de telle sorte qu'il se trouve sur les segment depart-arrivee
		...
		let i = 0;
		do {
			tabPoints[i] = cberect.getPoint(i/(nbPts*10));
			i++;
		} while(tabPoints[i-1].x != cberect.getPoint(1).x);
